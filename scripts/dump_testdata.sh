...
psql $PSQL_OPTIONS -a -c "\copy ( \
  ...
  (SELECT id, version, visible, user_id, tstamp, changeset_id, tags, geom FROM nodes n WHERE n.id IN (SELECT id FROM node_data)) \
    ORDER BY id, version \
...
psql $PSQL_OPTIONS -a -c "\copy ( \
  WITH way_ids AS (SELECT id FROM ways WHERE changeset_id = $CHANGESET_ID UNION \
   ...
   (SELECT id, version, visible, user_id, tstamp, changeset_id, tags, nodes FROM ways w WHERE w.id IN (SELECT id FROM way_ids)) \
   ORDER BY id, version \
